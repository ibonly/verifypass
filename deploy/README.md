...
| `EMAIL_API_URL` | HTTPS mailer subdomain origin, without `/send.php` |
| `CORS_ORIGINS` | Comma-separated exact origins; include dashboard, verification page and authorized embedding apps; no wildcard |
| `PROVIDER_MODEL_VERSION` | Validated model version label, e.g. `onnx-2026-07` |
| `SCHEMA_CHANGE_APPROVED` | `true` only after reviewing backward compatibility and backups |
| `CPANEL_SSH_HOST` | SSH hostname, without protocol or username |
| `CPANEL_SSH_PORT` | SSH port, usually `22` |
| `CPANEL_SSH_USER` | Restricted hosting account username |
| `CPANEL_RELEASE_ROOT` | One directory name under the account home, e.g. `verifypass`; no slashes |
...
For account `ACCOUNT` and release root `verifypass`, map subdomain document roots to:

```text
app.example.com     /home/ACCOUNT/verifypass/current/dashboard
verify.example.com  /home/ACCOUNT/verifypass/current/verify
mail.example.com    /home/ACCOUNT/verifypass/current/mailer/public
```
...
Create `/home/ACCOUNT/verifypass/shared/email-config.php` using the structure in `email-api/config.example.php`. Configure the actual SMTP account, HMAC key, HTTPS dashboard URL and branding; keep `env=production`, `require_https=true`, and `enable_render_endpoint=false`. Set its permissions to `0600`. Set `rate_limit.file` to an absolute path inside the same `shared` directory, which must be writable by the account's PHP process. This preserves replay/rate-limit state across releases and rollbacks.
...
- cPanel smoke failure: the previous shared symlink is restored automatically. A failed first release removes the new symlink because no previous release exists. Staged releases and archives are retained for diagnosis; prune them under an explicit retention policy, never delete the current release or private shared state.
- Runner termination during promotion: inspect `current` and `incoming/RELEASE.previous`. The remote script supports an ownership-checked rollback: run `bash deploy/cpanel-remote.sh rollback verifypass RELEASE` as the hosting account. It refuses to undo a newer unrelated release.
