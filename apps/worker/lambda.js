...
const { defaultEvidenceKey, runVerification } = require("./src/pipeline");
const { sendWebhook } = require("./src/webhookDispatcher");
const { failStuckSubmitted, reclaimStaleJobs } = require("./src/watchdog");
...
function getProvider() {
  if (!providerInstance) {
    ...
    const { createProviderForLambda } = require("./index");
...
async function sqsEnqueue(type, payload, opts = {}) {
  const { enqueue } = require("../api/src/services/jobService");
